...
# The same as delta_restore_over_previous_test, but the newer backup is an incremental one. The
# restore then walks the chain down to the full backup, and the files that the older restore
# already has right must not be fetched from either backup of the chain.

. /tmp/tests/test_functions/prepare_config.sh
prepare_config "/tmp/configs/delta_restore_over_previous_delta_test_config.json"
...
# The newer backup has to be an incremental one, otherwise this test is the same as the other.
if ! grep -q "Delta from" /tmp/delta_restore.log; then
  echo "Error: the newer backup was not an incremental one"
  exit 1
fi
...
echo "Delta restore of a delta backup over a previous restore success!!!!!!"
